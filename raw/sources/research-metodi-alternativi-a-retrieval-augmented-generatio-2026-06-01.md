...
Mentre il RAG tradizionale si basa su [[Vector Database]] e [[Hybrid Search]] per recuperare chunk di testo, il **GraphRAG** (o KG-RAG) integra un **knowledge graph** per arricchire il contesto con relazioni strutturate tra entità [17][18][19].
...
1. **Per knowledge base dinamiche e in rapida evoluzione**: RAG tradizionale (con [[Hybrid Search]] e [[Reranker]]) rimane la scelta più pragmatica.
...
5. **Per massima qualità possibile**: CPT → Fine-tuning → RAG (pipeline completa) con [[Embedding Model]] ottimizzati e reranking.
